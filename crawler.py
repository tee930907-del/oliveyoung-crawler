...
# ──────────── 상수 ────────────
# 2026-04 사이트 리뉴얼('lavender'): 페이지 기반 → 커서 기반 API로 교체됨
REVIEW_API_URL = "https://m.oliveyoung.co.kr/review/api/v2/reviews/cursor"
PAGE_SIZE = 10
MAX_PAGES = 200
...
def extract_reviews_from_json(data, depth=0):
    ...
    if isinstance(data, dict):
        for key in [
            "goodsReviewList",  # lavender /cursor 응답
...
def try_parse_review_dict(item):
    """딕셔너리에서 리뷰 정보 파싱 (lavender /cursor 스키마 + 레거시 fallback)"""
    ...
    # 리뷰 내용
    for k in [
        "content", "reviewContent", "contText", "reviewText",
    ...
    # 도움수 (lavender: usefulPoint, recommendCount)
    for k in [
        "usefulPoint", "recommendCount", "helpCount", "helpful",
        "likeCount", "likeCnt", "helpCnt"
    ]:
        if k in item and item[k] is not None:
...
def crawl_reviews(
    ...
    # max_pages는 의미를 보존: 페이지 사이즈 * max_pages 만큼 시도 후 안전 종료
    for page_idx in range(1, max_pages + 1):
        # zod 스키마 (1447 청크 기준): 요청 필드는 cursorId/cursorScore/cursorCount + size
        # (응답에서만 nextCursorId/nextCursorScore/nextCursorCount 로 내려옴)
        req_body: dict = {
            "goodsNumber": goods_no,
            "size": PAGE_SIZE,
            "sortType": sort_type or "USEFUL_SCORE_DESC",
            "reviewType": "ALL",
        }
        if cursor_id is not None:
            req_body["cursorId"] = cursor_id
        if cursor_score is not None:
            req_body["cursorScore"] = cursor_score
        if cursor_count is not None:
            req_body["cursorCount"] = cursor_count
        ...
        try:
            ...
            page_reviews = extract_reviews_from_json(payload)

            # 다음 커서 추출
            cursor_id = payload.get("nextCursorId")
            cursor_score = payload.get("nextCursorScore")
            cursor_count = payload.get("nextCursorCount")
            ...
            # 종료 조건: hasNext=false 또는 cursor 미존재
            if not has_next or cursor_id is None:
                log(f"✅ 리뷰 끝! (페이지 {page_idx}, 누적 {len(all_reviews)}개)")
                break
